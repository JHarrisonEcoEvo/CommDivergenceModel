...
model = function(sandbox,mode,sensitivity){
  running = TRUE
  require("MCMCpack")
      ...
      for(i in 1:length(sandbox)){
        for(j in 1:length(sandbox)){
          if(i != j){
            ...
            m=m+1
            #new smart function to prevent it for running waaay longer than it needs to
            if(mode == "smart"){
              #smart mode enabled time to check div to see how much is 1
              for(o in 1:sensitivity){
                if(div[m] == div[m-o] && div[m] == 1){
                  #set the endstep to the max non 1 thing
                  endstep = m-o
...
plotpoints = seq(from = 0, to = 700000, by=100)
